...
\lecture{General Introduction to Digital Signal Processing}{}
...
\end{equation}
where $\mathbf{n}$ is an $N$-dimensional random vector, $\boldsymbol{\mu}$ is the mean vector, and $\Sigma$ is the covariance matrix. Approximate $n(t)$ as a stationary Gaussian random process with zero mean $\mu=0$, which is a good approximation for LIGO noise. 
...
Therefore we can simplify the likelihood by working in frequency domain. $n^T \Sigma^{-1} n = n^T F^T (F^T)^{-1} \Sigma^{-1} F^{-1} F n = \tilde{n}^T \Lambda^{-1} \tilde{n}$, where $\tilde{n} = F n$ is the discrete Fourier transform of $n$. The likelihood becomes
    \begin{equation}
        P(\tilde{\mathbf{n}}) = \frac{1}{(2\pi)^{N/2} |\Lambda|^{1/2}} \exp\left(-\frac{1}{2} \tilde{\mathbf{n}}^T \Lambda^{-1} \tilde{\mathbf{n}}\right)
    \end{equation}
...
For quasi-circular binary black hole waveforms, there are 15 dimensional parameter space: 2 masses, 6 spin components, 3 sky locations (distance, right ascension, declination), and 2 orientations, 1 polarization and 1 time. Another 2 if considering eccentricity (eccentricity and anomaly).

In such a high dimensional parameter space, it is impossible to compute the likelihood on a grid. Instead, we use sampling methods, such as Markov Chain Monte Carlo (MCMC) or Nested Sampling, to explore the parameter space and estimate the posterior distribution of the parameters given the data.
...
Write the Bayes formula again but this time with $H$ explicitly
\begin{equation}
...
\begin{example}
    Massive graviton: $H_0$: $m_g = 0$ (GR), $H_1$: $m_g > 0$ (modified gravity). The Bayes factor is the ratio of the posterior and prior at $m_g = 0$. If the posterior at $m_g = 0$ is much smaller than the prior, then GR is disfavored.

    The modification from massive graviton is a frequency dependent dispersion relation: $E^2 = p^2 c^2 + m_g^2 c^4$,  $\Phi_\mathrm{MG}(f) = -\frac{\pi D m_g^2}{(1+z) f}$, where $D$ is the distance to the source and $z$ is the redshift. 
\end{example}
...
\end{equation}
where $h_0$ is the amplitude of the signal, $n_0$ is the amplitude of the noise, and $f_0$ is the frequency of the noise times signal. $T/f$ is the number of steps of that random walk. Therefore, we can detect the signal if $h_0 > \left(\frac{1}{f_0T}\right)^{1/2} n_0$. The longer we observe, the smaller the signal we can detect.

For $f_0 = 100$ s, $n_0 = 1e-21$, and $T = 1$ year, we can detect a signal with amplitude as small as $h_0 \sim 10^{-26}$.
...
\begin{align}
    N^2= \langle \hat{s}^2 \rangle|_{h=0} -\langle \hat{s} \rangle^2|_{h=0} (=0) =  \int_{-\infty}^{\infty} dt \int_{-\infty}^{\infty} dt' K(t) K(t') \langle n(t) n(t') \rangle = \\ \nonumber
...
\end{align}
Last line because $\hat{K}(f) = \int_{-\infty}^{\infty} dt K(t) e^{-2\pi i f t}$ is the Fourier transform of $K(t)$, and $\langle \hat{n}^\ast(f)\hat{n}(f') \rangle = \frac{1}{2} S_n(f) \delta(f-f')$ for stationary Gaussian noise.
...
Adding a term $\frac{1}{2}S_n(f)$ both on the numerator and denominator (by multipling it then dividing it) in the above equation.
Define the inner product
\begin{equation}
    \langle a, b \rangle =  \int_{-\infty}^{\infty} \frac{\tilde{a}^\ast(f) \tilde{b}(f)}{\frac{1}{2}S_n(f)} df  = 
    4 \Re \left[ \int_{0}^{\infty} \frac{\tilde{a}^\ast(f) \tilde{b}(f)}{S_n(f)} df \right]
\end{equation}
...
\end{equation}
This is called Weiner filter. 

\begin{remark}
    The matched-filtering SNR is then given by 
    \begin{equation}
        \frac{S}{N} = \frac{\langle d, h \rangle}{\sqrt{\langle h, h \rangle}} = 4 \Re \left[ \int_{0}^{\infty} \frac{\tilde{d}(f)\bar{\tilde{h}}^\ast(f)}{S_n(f)} df \right]
    \end{equation}
\end{remark}
\begin{remark}
    Optimal SNR = $\sqrt{\langle h, h \rangle} = \sqrt{4 \Re\int_0^\infty \frac{|\tilde{h}(f)|^2}{S_n(f)} df}$. This is the SNR we get when we know the true parameters of the signal. In practice, we don't know the true parameters, so we have to search over a template bank of possible waveforms, which leads to a reduction in the effective SNR due to the mismatch between the true signal and the templates.
\end{remark}
...
\[
    \frac{\partial \log \Lambda(\theta)}{\partial A}
    = \langle d, \bar{h}(\theta) \rangle - A \langle \bar{h}(\theta), \bar{h}(\theta) \rangle = 0
    \]

\[ A = \frac{\langle d, \bar{h}(\theta) \rangle}{\langle \bar{h}(\theta), \bar{h}(\theta) \rangle} \]

\[ \log \Lambda(\theta)|_{Max over A} = \frac{1}{2} \frac{\langle d, \bar{h}(\theta) \rangle^2}{\langle \bar{h}(\theta), \bar{h}(\theta) \rangle} = \frac{1}{2} \left( \frac{S}{N} \right)^2    \]

where $\Lambda(\theta) = \frac{\mathcal{L}(\theta)}{\mathcal{L}(0)}$ is the likelihood ratio between the signal hypothesis and the noise-only hypothesis.
...
Type I error: $P(\Lambda(d) > \eta | H_0) = \alpha$ (false positive)

Type II error: $P(\Lambda(d) \leq \eta | H_1) = \beta$ (false negative)

The meaning of the most powerful test is that it has the highest probability of correctly rejecting $H_0$ when $H_1$ is true (i.e., it maximizes the power of the test, which is $1 - \beta$, where $\beta$ is the probability of a Type II error).
...
\subsection*{$\chi^2$ distribution, Goodness of fitting, and new-SNR}
...
\subsection*{Procedure for searching for GW signals}
\begin{itemize}
    \item Build a template bank of waveforms that cover the parameter space of interest.
    \item Compute the matched-filtering SNR for each template against the observed data.
    \item Looking for coincidence of multiple detector triggers
    \item Use time shifts to construct a background distribution and estimate the false alarm rate (FAR) for candidate events.
\end{itemize}
